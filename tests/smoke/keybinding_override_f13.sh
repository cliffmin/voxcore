...
# Smoke test: override to F13 & Shift+F13 via KEYS
...
if [[ "$HOLD" != "f13" ]]; then
  echo "FAIL: HOLD expected f13, got '$HOLD'" >&2
  exit 1
fi
if [[ "$TOGGLE" != "shift+f13" ]]; then
  echo "FAIL: TOGGLE expected shift+f13, got '$TOGGLE'" >&2
  exit 1
fi

echo "OK: KEYS override to F13 works (HOLD=$HOLD, TOGGLE=$TOGGLE)"
